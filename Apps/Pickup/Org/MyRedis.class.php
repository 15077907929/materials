...
class MyRedis
{
    ...
    static function getInstance($conf=null)
    {
        ...
        {
            if($conf)
                self::$instance = new MyRedis( $conf );
            else
	...
	function get($key)
	{
		...
		return $this->jsonDecode($val);
	}

	function set($key, $val ,$expire = 0)
	{
        $value = $this->jsonEncode($val);
		if(intval($expire)>0)
    ...
    function lPop($key )
    {
	 ...
	 function lGet($key, $start = null, $end = null)
     {
         if ($start === null) //返回所有
         {
             ...
             foreach ($arr as $k => $v)
                 $arr[$k] = $this->jsonDecode($v);
             return $arr;
         }
         if ($end === null) //返回指定的
             return $this->jsonDecode($this->redis->lGet($this->prefix . $key, $start));
	...
	function sAdd($key,$value)
	{
		if(is_array($value))
		{
			...
			foreach($value as $v)
			{
				$c += $this->redis->sAdd($this->prefix.$key, $this->jsonEncode($v));
			}
			return $c;
		}
		return $this->redis->sAdd($this->prefix.$key, $this->jsonEncode($value));
	}
