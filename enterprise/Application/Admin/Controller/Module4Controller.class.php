...
class Module4Controller extends CommonController{	
	public function index(){	
		$method=(I('get.method')=='')?'add':I('get.method');	//请求方式
		$db=M('web_column');
		$p=(I('get.p')=='')?1:I('get.p');	//当前页
		$class1=I('get.class1');
		$count=M('web_download')->where('class1='.$class1)->count();	//总记录数		
		switch($method){
			case 'add':				
			...
			case 'list':		
				...
				$result_arr['data']=M('web_download')->field('id,title,com_ok,new_ok,updatetime')->where('class1='.$class1)->page($p.',2')->select();
