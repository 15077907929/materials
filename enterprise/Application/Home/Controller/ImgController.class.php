...
class ImgController extends CommonController{	
    public function img(){
		...
		switch($class_type){
			case 'base':
				$result_arr['img_list']=$db->field('id,title,imgurls')->where('class1='.I('get.class1'))->page($p.',2')->select();
				$count=$db->where('class1='.I('get.class1'))->count();	//总记录数
			break;			
			case 'second':
				$result_arr['img_list']=$db->field('id,title,imgurls')->where('class1='.I('get.class1').' and class2='.I('get.class2'))->page($p.',2')->select();
				$count=$db->where('class1='.I('get.class1').' and class2='.I('get.class2'))->count();	//总记录数
			break;
			case 'third':
				$result_arr['img_list']=$db->field('id,title,imgurls')->where('class1='.I('get.class1').' and class2='.I('get.class2').' and class3='.I('get.class3'))->page($p.',2')->select();
