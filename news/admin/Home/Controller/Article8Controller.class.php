...
class Article8Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wx0e4b7c0e577c1795');
		switch ($method) {
			case 'list':
				$this->assign('cur_nav',8);
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article8&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
	...
	public function pickup(){
		...
		switch ($method) {
			...
			break;
			case 'pick':
				$this->redirect(U('index.php?m=Pickup&c=PickupArts&a=pickupKeyword&art_keyword='.$_POST['art_keyword'].'&num='.$_POST['num']));
			break;
		}
	}
	...
	public function keyword(){
		...
		$res['list']=M('wx0e4b7c0e577c1795')->distinct(true)->where('art_keyword!=\'\'')->field('art_keyword')->select();
		foreach($res['list'] as &$val){
