...
class Article19Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wxb186ab78c6fc4347');
		switch ($method) {
			case 'list':
				$this->assign('cur_nav',19);
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article19&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
