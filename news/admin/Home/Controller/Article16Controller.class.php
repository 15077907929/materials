...
class Article16Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wx58eea459ca096f71');
		$this->assign('cur_nav',16);
		switch ($method) {
			case 'list':
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article16&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
