...
class Article92Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wx0a34426b2810c154');
		$this->assign('cur_nav',92);
		switch ($method) {
			case 'list':
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article92&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
