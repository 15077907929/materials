...
class Article25Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wxbc6bcc4a1462acba');
		$this->assign('cur_nav',25);
		switch ($method) {
			case 'list':
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article25&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
