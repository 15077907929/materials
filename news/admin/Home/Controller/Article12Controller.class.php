...
class Article12Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wx9760391858a138b1');
		switch ($method) {
			case 'list':
				$this->assign('cur_nav',12);
				$res['count']=$db->count();
			...
			case 'add':
				...
				$res['art_keyword']=M('Article12')->distinct(true)->where('art_keyword!=\'\'')->field('art_keyword')->select();
				$this->assign('res',$res);
				if(I('post.sub')){
					$arr=array(
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article12&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
	...
	public function keyword(){
		...
		$res['list']=M('Article12')->distinct(true)->where('art_keyword!=\'\'')->field('art_keyword')->select();
		foreach($res['list'] as &$val){
			$val['art_addtime']=M('Article12')->where('art_keyword!=\''.$val['art_keyword'].'\'')->field('art_addtime')->find()['art_addtime'];
		}
		$this->assign('res',$res);
