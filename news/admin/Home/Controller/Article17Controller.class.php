...
class Article17Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wxb859f1b55c6e882a');
		switch ($method) {
			case 'list':
				$this->assign('cur_nav',17);
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article17&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
