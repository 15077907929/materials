...
class Article20Controller extends CommonController {
    public function index(){
		$method = I('get.method') ? I('get.method') : 'list';
		$db=M('wx59753e94c9cf80bf');
		$this->assign('cur_nav',20);
		switch ($method) {
			case 'list':
				$res['count']=$db->count();
			...
			case 'add':
				...
				if(I('post.sub')){
					...
					if($db->add($arr)){
						$this->success('添加成功');
						$this->redirect(U('index.php?m=Home&c=Article20&a=index&method=list'));
					}else{
						$this->error('添加失败');
					}
				}
