...
<section class="container">
	...
	<div class="main">
		...
		@endforeach
		<form name="category" action="{{url('admin/category/'.$field->id)}}" method="post" onsubmit="return check();">
			{{csrf_field()}}
			...
			<table>				
				<tr>
					...
					<td>
						<select name="pid">
							...
							@foreach($cate as $val)
								<option @if($val->id==$field->pid) selected @endif value="{{$val->id}}">{{$val->name}}</option>
							@endforeach
						</select>
					</td>
				</tr>
				...
				<tr>
					<td>关键词：</td>
					<td><input type="text" name="keywords" value="{{$field->keywords}}" /></td>
				</tr>
				<tr>
					<td>描述：</td>
