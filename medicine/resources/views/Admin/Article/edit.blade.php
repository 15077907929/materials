...
<section class="container">
	<div class="breadcrumbs">
		...
		<div class="breadcrumb">
			...
			<a href="{{url('admin/index')}}">首页</a> &gt; 文章管理 &gt; 编辑文章
	...
	<div class="main">
		...
		@endforeach
		<form name="article" action="{{url('admin/article/'.$field->id)}}" method="post" onsubmit="return check();">
			{{csrf_field()}}
			<input type="hidden" name="_method" value="put" />
			<table>
				<tr>
					<td width="18%"><font color="#f00">*</font>文章标题：</td>
					<td><input type="text" name="title" value="{{$field->title}}" /></td>
				</tr>				
				<tr>
					...
					<td>
						<select name="cate_id">
							...
							@foreach($cate as $val)
								<option @if($val->id==$field->cate_id) selected @endif value="{{$val->id}}">{{$val->name}}</option>
								@foreach($val['sub'] as $v)
									<option @if($v->id==$field->cate_id) selected @endif value="{{$v->id}}">|-{{$v->name}}</option>
								@endforeach
							@endforeach
						</select>
					</td>
				...
				<tr>
					<td>排序：</td>
					<td><input type="text" name="no_order" value="{{$field->no_order}}" /></td>
				</tr>				
				<tr>
					<td>关键词：</td>
					<td><input type="text" name="tag" value="{{$field->tag}}" /></td>
				</tr>
				<tr>
					<td>文章摘要：</td>
					<td><textarea name="description">{{$field->description}}</textarea></td>
				</tr>
				<tr>
					<td>文章编辑：</td>
					<td><input type="text" name="editor" value="{{$field->editor}}" /></td>
				</tr>
				<tr>
					<td>文章内容：</td>
					<td>
						...
						<script id="editor" name="content" type="text/plain" style="line-height:20px;width:860px;height:360px;">
						{!!$field->content!!}
						</script>
