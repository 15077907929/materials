...
class upfile {
	...
	var $savename;
	// 原文件保存路径
	var $savepath = '/opt/lampp/htdocs/enterprise.hd/Uploads';
	//水印保存路径
	var $waterpath = '/opt/lampp/htdocs/enterprise.hd/Uploads/watermark';
	// 文件格式限定，为空时不限制格式
	var $format = "";
	// 覆盖模式
	var $overwrite = 1;
	...
	function upfile($format = '',$path = '',$maxsize = 0, $over = 0) {
		if (empty($path)) {
			...
			if (!file_exists($water)){
				!$this->make_dir($water)&& $this->halt($water.'创建水印目录失败');
			}
