...
class FriendlinkController extends CommonController {	
	public function friendlink(){
		...
		switch ($method) {
            ...
            case 'add':
				if($_POST){
					...
					if($db2->add($data)){
						echo '<script type="text/javascript"> alert("操作成功"); location.href="index.php?m=Admin&c=friendlink&a=friendlink&class1='.$_POST['class1'].'"; </script>';
					}					
				}
				$res['info']=$db->where('id='.$_GET['class1'])->find();
		...
		$this->assign('res',$res);
		$this->display('friendlink_'.$method);
	}
}